...
class LogOffpolicyTrainer(BaseTrainer):
    ...
    def test_step(self) -> Tuple[Dict[str, Any], bool]: 
        ...
        if self.epoch % 1 == 0:
            ...
            # print("Confidence",compute_confidence(test_result["episode_states"],self.config))
            confidences = compute_confidence(test_result["episode_states"],self.config)
            print(confidences)
            log_data = {f"test/{rule}_{player}":confidences[player][rule] for player in confidences for rule in confidences[player]}
            log_data["test/env_step"] = self.env_step
            self.logger.write(f"test/env_step", self.env_step, log_data)
...
def logoffpolicy_trainer(*args, **kwargs) -> Dict[str, Union[float, str]]:  # type: ignore
    ...
    """
    return LogOffpolicyTrainer(*args, **kwargs).run()


logoffpolicy_trainer_iter = LogOffpolicyTrainer
